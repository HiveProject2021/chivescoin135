...
from chives.util.db_version import set_db_version
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        if updated_target:
            print(
                f"To change the XCC destination addresses, edit the `xch_target_address` entries in"
...
def init(
    ...
    if create_certs is not None:
        ...
        return chives_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)
