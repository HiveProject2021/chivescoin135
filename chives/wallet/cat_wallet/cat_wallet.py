...
from chives.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from chives.wallet.cat_wallet.lineage_store import CATLineageStore
from chives.wallet.lineage_proof import LineageProof
from chives.wallet.payment import Payment
from chives.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        # For now, give the wallet a temporary name "Token Wallet" until we get the tail hash
        original_name = name
        if name is None:
            name = "Token Wallet"
    ...
    async def get_confirmed_balance(self, record_list: Optional[Set[WalletCoinRecord]] = None) -> uint64:
        ...
        self.log.info(f"Confirmed balance for Token Wallet {self.id()} is {amount}")
    ...
    async def coin_added(self, coin: Coin, height: uint32):
        ...
        self.log.info(f"Token Wallet has been notified that {coin} was added")
