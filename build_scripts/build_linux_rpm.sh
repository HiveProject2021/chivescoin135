...
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/chives/"
ln -s ../../opt/chives/chives "dist/$CLI_RPM_BASE/usr/bin/chives"
...
fpm -s dir -t rpm \
  ...
  --name chives-blockchain-cli \
  --license Apache-2.0 \
  --version "$CHIVES_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "Chives is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
