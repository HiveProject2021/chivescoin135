...
from chives.cmds.chives import cli
from chives.cmds.keys import delete_all_cmd, generate_and_print_cmd, show_cmd, sign_cmd, verify_cmd
from chives.util.config import load_config
from chives.util.file_keyring import FileKeyring
from chives.util.keychain import DEFAULT_USER, DEFAULT_SERVICE, Keychain, generate_mnemonic
from chives.util.keyring_wrapper import DEFAULT_KEYS_ROOT_PATH, KeyringWrapper, LegacyKeyring
...
def empty_keyring():
    with TempKeyring(user="user-chives-1.8", service="chives-user-chives-1.8") as keychain:
        yield keychain
...
class TestKeysCommands:
    ...
    def test_show(self, keyring_with_one_key):
        """
        Test that the `chives keys show` command shows the correct key.
        """
    ...
    def test_show_mnemonic(self, keyring_with_one_key):
        """
        Test that the `chives keys show --show-mnemonic-seed` command shows the key's mnemonic seed.
        """
    ...
    def test_generate_and_print(self):
        """
        Test the `chives keys generate_and_print` command.
        """
    ...
    def test_sign(self, keyring_with_one_key):
        """
        Test the `chives keys sign` command.
        """
    ...
    def test_sign_non_observer(self, keyring_with_one_key):
        """
        Test the `chives keys sign` command with a non-observer key.
        """
    ...
    def test_verify(self):
        """
        Test the `chives keys verify` command.
        """
    ...
    def test_derive_search(self, tmp_path, keyring_with_one_key):
        """
        Test the `chives keys derive search` command, searching a public and private key
        """
    ...
    def test_derive_search_wallet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `chives keys derive search` command, searching for a wallet address
        """
        ...
        assert (
            result.output.find(
                (
                    ...
                    "xch1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs8taffd (HD path: m/12381/9699/2/30)"
                )
            )
            != -1
        )
    ...
    def test_derive_search_failure(self, tmp_path, keyring_with_one_key):
        """
        Test the `chives keys derive search` command with a failing search.
        """
    ...
    def test_derive_search_hd_path(self, tmp_path, empty_keyring, mnemonic_seed_file):
        """
        Test the `chives keys derive search` command, searching under a provided HD path.
        """
    ...
    def test_derive_wallet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `chives keys derive wallet-address` command, generating a couple of wallet addresses.
        """
    ...
    def test_derive_child_keys(self, tmp_path, keyring_with_one_key):
        """
        Test the `chives keys derive child-keys` command, generating a couple of derived keys.
        """
    ...
    def test_migration_not_needed(self, tmp_path, setup_keyringwrapper, monkeypatch):
        """
        Test the `chives keys migrate` command when no migration is necessary
        """
    ...
    def test_migration_full(self, tmp_path, setup_legacy_keyringwrapper):
        """
        Test the `chives keys migrate` command when a full migration is needed
        """
        ...
        current_version_str = pkg_resources.get_distribution("chives-blockchain").version
    ...
    def test_migration_incremental(self, tmp_path, keyring_with_one_key, monkeypatch):
        ...
        from chives.util import keyring_wrapper
    ...
    def test_migration_checked(self, tmp_path, monkeypatch):
        ...
        assert pkg_resources.get_distribution("chives-blockchain").version == "1.2.11.dev123"
