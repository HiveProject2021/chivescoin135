...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            chives_coins: List[Coin] = all_coins[None]
            ...
            chives_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                chives_requested_payments, chives_coins
            )
            chives_announcements: List[Announcement] = Offer.calculate_announcements(chives_requested_payments)
            chives_secured_bundle: SpendBundle = generate_secure_bundle(chives_coins, chives_announcements, 1000)
            ...
            new_offer = Offer.aggregate([chives_offer, red_offer])
