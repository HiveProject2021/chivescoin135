...
export default function Block() {
  ...
  const poolReward = mojoToChives(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToChives(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const chivesFees = blockRecord.fees !== undefined
    ? mojoToChives(blockRecord.fees)
    : '';
