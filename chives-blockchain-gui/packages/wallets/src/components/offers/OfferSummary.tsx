...
import { type OfferSummaryRecord  } from '@chives/api';
import {
  Flex,
  FormatLargeNumber,
  StateColor,
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount = makerEntries.length > 0 ? (['xch', 'txch'].includes(makerEntries[0][0].toLowerCase()) ? mojoToChives(makerEntries[0][1]) : mojoToCAT(makerEntries[0][1])) : undefined;
  const takerAmount = takerEntries.length > 0 ? (['xch', 'txch'].includes(takerEntries[0][0].toLowerCase()) ? mojoToChives(takerEntries[0][1]) : mojoToCAT(takerEntries[0][1])) : undefined;
